...
// CODE HERE
const myStrongest = monstersInYourPocket.filter(element => element = element.CP > 200);
...
const taxedTotals = orders.map(element => element.price * (element.tax + 1));
console.log(taxedTotals)
...
const bobsTotal = purchases.reduce((runningTotal, curElement) => {
  if(curElement.owner === `Bob`){
    runningTotal += curElement.price;
  }
  return runningTotal;
}, 0);
