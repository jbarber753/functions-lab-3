...
const halvedArr = numbers.map(element => element / 2);
...
const newCities = cities.filter(element => element[element.length - 1] === `i`);
...
const riceTotal = foods.reduce((total, element) => {
  if(element.tags.includes(`rice`)){
    total += element.price;
  }
  return total;
}, 0);
